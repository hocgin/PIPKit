...
class AVPPIPUIView: UIView, AVPPIPUIKitUsable {
...
struct AVPPIPUIViewRepresentable: UIViewRepresentable {
    
    // 这里的 MyCustomUIView 是你之前创建的自定义 UIView
    func makeUIView(context: Context) -> AVPPIPUIView {
        AVPPIPUIView()
    }
    
    // 2. 更新 UIView 的数据或状态（可以为空，取决于你是否需要动态更新）
    func updateUIView(_ uiView: AVPPIPUIView, context: Context) {
        // 在这里更新自定义 UIView
